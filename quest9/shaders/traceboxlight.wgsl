...
fn lamberShader(uv: vec2i, spt: vec3f, rdir: vec3f) {
  ...
  if (hitInfo.x > 0) {
    ...
    // HARD SHADOW
    let shadowBias = 0.01;
    let shadowRayOrigin = hitPt + normal * shadowBias;
    let shadowRayDir = normalize(lightPos - hitPt);
    let shadowHit = rayBoxIntersection(shadowRayOrigin, shadowRayDir);
    let lightDist = length(lightPos - hitPt);

    var visibility = 1.0;
    if (shadowHit.x > 0.0 && shadowHit.x < lightDist) {
      visibility = 0.0; // In shadow
    }

    // SOFT SHADOW (basic N-sample approximation)
    let samples = 4;
    var softShadowFactor = 0.0;
    let areaRadius = 0.1;
    for (var i = 0; i < samples; i++) {
      let offset = vec3f(
        rand(hitPt.xy + vec2f(f32(i), 0.0)) - 0.5,
        rand(hitPt.yz + vec2f(0.0, f32(i))) - 0.5,
        rand(hitPt.zx + vec2f(f32(i), 0.5)) - 0.5
      ) * areaRadius;
      let sampleLightPos = lightPos + offset;
      let sampleDir = normalize(sampleLightPos - hitPt);
      let sampleHit = rayBoxIntersection(shadowRayOrigin, sampleDir);
      let sampleDist = length(sampleLightPos - hitPt);
      if (sampleHit.x <= 0.0 || sampleHit.x >= sampleDist) {
        softShadowFactor += 1.0;
      }
    }
    softShadowFactor /= f32(samples);

    let softness = 0.7; // 0.0 = hard shadow, 1.0 = full soft shadow
    let shadowStrength = mix(visibility, softShadowFactor, softness);
    diffuse *= lightInfo.intensity * shadowStrength;
...
fn rand(p: vec2f) -> f32 {
  let k1 = 50.0;
  let k2 = 161.0;
  return fract(sin(dot(p, vec2f(k1, k2))) * 43758.5453123);
}
